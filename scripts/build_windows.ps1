...
$releaseContractFiles = @(
    @{ Path = "app.py"; Pattern = 'APP_VERSION = "1\.13\.0"' },
    @{ Path = "lumafetch\__init__.py"; Pattern = '__version__ = "1\.13\.0"' },
    @{ Path = "installer\LumaFetch.iss"; Pattern = '#define MyAppVersion "1\.13\.0"' },
    @{ Path = "installer\LumaFetch.iss"; Pattern = 'OutputBaseFilename=LumaFetch-Setup-1\.13\.0' }
)
...
if ($appVersion.FileVersion -notlike "1.13.0*") {
...
if ($installerVersion.FileVersion -notlike "1.13.0*") {
