...
$releaseContractFiles = @(
    @{ Path = "app.py"; Pattern = 'APP_VERSION = "1\.13\.1"' },
    @{ Path = "lumafetch\__init__.py"; Pattern = '__version__ = "1\.13\.1"' },
    @{ Path = "installer\LumaFetch.iss"; Pattern = '#define MyAppVersion "1\.13\.1"' },
    @{ Path = "installer\LumaFetch.iss"; Pattern = 'OutputBaseFilename=LumaFetch-Setup-1\.13\.1' }
)
...
if ($appVersion.FileVersion -notlike "1.13.1*") {
...
if ($installerVersion.FileVersion -notlike "1.13.1*") {
